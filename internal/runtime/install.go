...
// InstallClawDeckX downloads a ClawDeckX release binary into the runtime overlay.
// The downloadURL should point to the linux-amd64 or linux-arm64 binary.
// progressFn receives progress updates (can be nil).
func (m *Manager) InstallClawDeckX(ctx context.Context, downloadURL string, progressFn func(updater.ApplyProgress)) error {
	if err := m.EnsureDirs(); err != nil {
		return err
	}

	dir := m.binaryDir(ComponentClawDeckX)
	binPath := filepath.Join(dir, "clawdeckx")
	tmpPath := binPath + ".tmp"

	// Read current manifest for prev_version
	prevVersion := ""
	if mf, _ := m.ReadManifest(ComponentClawDeckX); mf != nil {
	...
	logger.Log.Info().
		...
		Str("path", binPath).
		Msg("ClawDeckX runtime overlay installed")
...
func parseVersionOutput(s string) string {
	s = trimAll(s)
	// Common patterns: "v0.0.18", "0.0.18", "ClawDeckX v0.0.18"
	for _, line := range splitLines(s) {
		...
		for _, prefix := range []string{"clawdeckx ", "openclaw ", "version ", "v"} {
